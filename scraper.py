...
sheet_url = 'https://docs.google.com/spreadsheets/d/1xKu26F7SLk0b6OMh5iQ5520Gt4xU9Wm_a1a5mna_H8I/edit#gid=0'
sheet = gc.open_by_url(sheet_url).worksheet('Sheet2')
log("✅ Connected to NEW Google Sheet.")
...
for idx, acc in enumerate(accounts):
    ...
    if login_to_screener(session, acc['username'], acc['password']):
            ...
            if acc['add_classification']:
                ...
                for i, row in enumerate(rows):
                    ...
                    if len(cols) > 1:
                        ...
                        df.iloc[i - 1, -1] = f'=HYPERLINK("https://www.screener.in{name_link}", "{name_column.text.strip()}")'

                df = df[[*df.columns[:-2], 'Classification', 'Hyperlink']]
            else:
                df = df.iloc[:, :18]

            all_data += [df.columns.tolist()] + df.values.tolist() + [[""] * len(df.columns)]
        ...
        log(f"✅ Data written to range {acc['range']}")
...
requests.get(apps_script_url)
log("✅ Google Apps Script triggered.")
